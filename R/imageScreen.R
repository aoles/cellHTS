...
imageScreen <- function (x, ar=3/5, zrange, map=FALSE, anno) {
  ...
  if (!missing(anno)) {
    if(length(anno)!=length(x$score) & (map))
       stop("Argument 'anno' is not valid! It should be a vector with the same length as 'x$score').")
  }else{## if !missing anno
    if(x$state["annotated"]){
       if ("GeneSymbol" %in% names(x$geneAnno))
         anno <- x$geneAnno$GeneSymbol
       else

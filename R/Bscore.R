...
## if model.log=TRUE, the model is fitted on log2 scale, but the output
## results (normalized values, overall effect, residuals and rowcol.effects) are given in the original scale.

Bscore <- function(x, adjustPlateMedian=TRUE, model.log=FALSE, scale=TRUE, save.model=FALSE) {

  xn <- array(as.numeric(NA), dim=dim(x))
  ...
  if (save.model) {
    ...
    if (adjustPlateMedian)
      overall.effects = array(as.numeric(NA), dim=c(1, dim(xn)[2:4]))
  }
  ...
  for(p in 1:(dim(x)[2])) {
    ...
    for(r in 1:(dim(x)[3])) {
      for(ch in 1:(dim(x)[4])) {
        ...
        if(!all(is.na(y))) {
          ysamp[!samples]=NA
          ysamp = matrix(ysamp, ncol=x$pdim["ncol"], nrow=x$pdim["nrow"], byrow=TRUE)
          y = matrix(y,         ncol=x$pdim["ncol"], nrow=x$pdim["nrow"], byrow=TRUE)
          ...
          xn[, p, r, ch] = if (scale) 
            as.vector(t(res))/mad(m$residuals, na.rm=TRUE) 
          else 
            as.vector(t(res))

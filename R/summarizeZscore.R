summarizeZscore=function(x, summary="min") {
...
# the final z-scores are just the summarized replicate values
  x$score = avr # the final z-scores are just the summarized replicate values
  x$state["scored"] = TRUE
  return(x)
}

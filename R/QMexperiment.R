QMexperiment = function(x, path, con, posControls, negControls, isTwoWay=FALSE, namePos) {
  ...
  actCtrls = vector("list", length=nrCh)
  inhCtrls = vector("list", length=nrCh)
  ...
  if(x$state["configured"]) {
   for (ch in 1:nrCh) {
    if(isTwoWay){
      if (!(posControls$act[ch] %in% c(NA, ""))) 
        actCtrls[[ch]] = which(regexpr(posControls$act[ch], wellAnno, perl=TRUE)>0)
      if (!(posControls$inh[ch] %in% c(NA, "")))  
  ...
  hasLessCh=FALSE
  if (x$state["normalized"]) hasLessCh = dim(x$xraw)[4] > dim(x$xnorm)[4]

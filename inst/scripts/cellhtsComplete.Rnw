...
Wolfgang Huber~\cite{Boutros2006}. This report explains all the steps necessary to run a
...
\textit{cellhtsComplete.Rnw} in the \textit{scripts} directory of the package,
...
if (!("cellhts.bib" %in% dir()) )
  system(sprintf("cp %s/cellhts.bib .", system.file("doc", package="cellHTS")))
...
in Figure~\ref{cellhtsComplete-boxplotzscore}.
%
...
@ 
%
\myincfig{cellhtsComplete-boxplotzscore}{0.5\textwidth}{Boxplots of $z$-scores 
...
rnwPath <- system.file("doc/Rnw", package="cellHTS")
setwd(rnwPath)
...
<<obsolete GO ids>>=
#obsolete = c("GO:0005489", "GO:0005659", "GO:0005660")
...
%
<<cat3>>=
stats <- x$score[ sel & (names(x$score) %in% colnames(categs)) ]
@
There are some replicated probes in \Robject{stats}. We will handle this by taking the maximum value between replicate probes (non-conservative approach):
<<handle replicates>>=
...
Figure~\ref{cellhtsComplete-volcano}. For a given category, the $p$-value is calculated from the 
...
Figure~\ref{cellhtsComplete-volcano} are listed in Table~\ref{tab:enrichedGoCateg}.

 
%
\input{cellhts-enrichedGoCateg} 
%
\myincfig{cellhtsComplete-volcano}{0.5\textwidth}{Volcano plot of the $t$-test
...
%
%
\myincfig{cellhtsComplete-scoresComparison}{\textwidth}{Scored 
...
@ 
%
Figure~\ref{cellhtsComplete-scoresComparison} shows the scatterplot between Boutros 
...
In fact, Figure~\ref{cellhtsComplete-scoresComparison} evidenciates how the scored values
...
%------------------------------------------------------------
\myincfig{cellhtsComplete-transfplots}{0.95\textwidth}{Comparison between untransformed 
...
Figure~\ref{cellhtsComplete-transfplots} compares these properties for

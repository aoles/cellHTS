...
Below, we apply \Rfunction{normalizeChannels} to take the log ratio $R_2/R_1$ using
the data values in \Robject{x\$xraw}, and then use plate median normalization to remove plate-to-plate 
variations:
%
<<Ratio R2/R1>>=
x <- normalizeChannels(x, adjustPlates="median")
@ 
%
...
@ 
%
\myincfig{twoChannels-boxplotzscore}{0.7\textwidth}{Boxplots of $z$-scores 

...
namely, median and median absolute deviation (MAD). Moreover, we only considered the wells containing ``sample'' for estimating $\hat{\mu}$ and $\hat{\sigma}$.
As the values
...
\item \verb=finalWellAnno_r1_ch1= and \verb=finalWellAnno_r2_ch1= give the final well 

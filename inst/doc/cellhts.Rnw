...
data2003 = read.table(paste(dataPath, "Analysis2003.txt", sep="/"), header=TRUE, 
as.is=TRUE, sep="\t")
@ 
% 
...
\textit{et al.}' scores and our scores in each of the 384-well plates.
...
instead of mean and standard deviation), and a estimation of mad over 

...
with those where the R packages were compiled, and that they are
...
Naturally, you will want to explore the \Rpackage{cellHTS} package and
see whether it is worthwhile before facing an extensive installation
struggle, hence we have made available the result of the annotation
with \Rpackage{biomaRt}, described below, as a precomputed R object:
<<>>=
data("bdgp42biomart")
x$geneAnno = bdgp42biomart
@ 

%------------------------------------------------------------------
\subsubsection{Using biomaRt to annotate the target genes online}
%------------------------------------------------------------------
The commands in this section are optional, you do not need them to
complete this example analysis. They demonstrate how to obtain the
dataframe \Robject{bdgp42biomart} that we have loaded above by
querying the online webservice \textit{BioMart} and through it the
Ensembl genome annotation database~\cite{Ensembl2006}.

Let's check which BioMart databases it currently covers:
%
<<listMarts>>=
listMarts()
@
%
In this example, we will use the Ensembl database~\cite{Ensembl2006}, from which we select the
...
especially from gene ID to GO categories~\cite{GO}.  Below, we add the results

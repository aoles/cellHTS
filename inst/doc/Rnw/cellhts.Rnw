...
corresponding genes in cultured \textit{Drosophila} $K_{c167}$ 
...
is indicative of ATP levels. Although this example data corresponds to a
...
annotation from public databases~\cite{biomaRt2005}. After loading the 
R package \Rpackage{biomaRt}, we can check which are the 
...
controls~\cite{Oldenburg1999}, the boxplots with raw and normalized intensities for the different plates,
...
%------------------------------------------------------------
We would like to see whether there are Gene Ontology categories~\cite{GO}
overrepresented among the probes with a high score. For
this we use the category analysis from Robert Gentleman's
\Rpackage{Category} package~\cite{GentlemanCategories}. Similar analyses could be done for other

...
z_{k}  &=& \sqr{\frac{1}{n_{\mbox{\scriptsize rep}}} \sum_{i} z_{ki}^2} \label{eq:scoresSummary}.
\end{eqnarray}
...
choices of summary function between replicates are the minimum and the
maximum. In the first case, the analysis would be particularly

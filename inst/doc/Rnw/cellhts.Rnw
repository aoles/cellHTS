...
The \emph{Content} column can contain one of the 
following: \textit{sample} (for wells that
...
\label{sec:norm}
%------------------------------------------------------------ 
The function \Rfunction{normalizePlates} can be called to adjust for plate effects. The \Robject{normalizationMethod}  allows to perform different normalizations. For example, if it is set to \Robject{"median"}, the function \Rfunction{normalizePlates} adjusts for plate
...
each gene. One option is to take the average of the $z$-scores from the two
replicates:
\begin{eqnarray}
z_{ki} &=& \pm \frac{x'_{ki}-\hat{\mu}}{\hat{\sigma}} \label{eq:defz} \\
z_{k}  &=& \mean_{i} z_{ki} \label{eq:scoresSummary}.
\end{eqnarray}
Here $\hat{\mu}$ and $\hat{\sigma}$ are estimators of location and
scale of the distribution of $x'_{ki}$.  We use robust estimators,
namely, median and median absolute deviation (mad).  As the values
...
replicates for probe $k$. Depending on the intended stringency of
the analysis, other plausible choices of summary function between replicates 
are the min, and the maximum. In the first case, the analysis would be particularly conservative: all replicate values have to be high in
order for $z_{k}$ to be high. 
In order to compare our results with those obtained in the paper of Boutros \textit{et al.}~\cite{Boutros2004}, we have chosen to consider the mean as a summary.
...
\Rpackage{biomaRt}.  It is optional, you can move on to the next
...
gene annotation (you do not need to run the following two lines if
you want to get the annotation online as described in the remainder 
of this section).
...
%
<<biomaRt, eval=TRUE, echo=FALSE, results=hide>>=
Sweave("biomart.Rnw")
@ 
<<bdgpbiomart2, eval=TRUE>>=
...
xlog = normalizePlates(x, normalizatioMethod="median", transform=log2)
...
%------------------------------------------------------------
\section{Appendix: Multiple channel experiments}
%------------------------------------------------------------

The \Rpackage{cellHTS} includes an example of a two-color screen performed with \textit{D. melanogaster} cells. Two reporters were used. The first reporter is specific for ...., while the second reporter is indicative of 


reporter $R1$ could monitor the total number of viable cells in a
well, another reporter $R2$ the activity of a particular
pathway. 
Such experimental setups are typically used in screens for
signaling pathway components, where a pathway-inducible readout is
normalized against a constitutive reporter. 
In this way, it becomes possible to
distinguish between changes in the readout caused by depletion of
specific pathway components versus changes in the overall cell number.
The \textit{cellHTS} package provides a function to perform plate
median normalization on the ratios $R2/R1$, which can be used as a
measure of pathway activity per cell.

This approach can be expanded to other types of reporters and to
experiments with more than two reporters. Taking the in-built
normalization functions of \textit{cellHTS} as a template, one can
employ the extensive statistical modeling capabilities of the R
programming language to consider more general analysis strategies that
are adapted to the biological system and the question of
interest.

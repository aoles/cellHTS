...
the replicates after scaling by the plate median\footnote{The
  \Rpackage{cellHTS} also provides a function called \Rfunction{calcZscore} that
first summarizes the normalized replicate values, and then calculates the 
$z$-scores. However, in the present example, we prefer to have the normalized and scored
replicates in \Robject{x\$xnorm}, and use them for constructing the quality report.}:
%
...
Using the above function, we summarize the scored replicates, obtaining a single $z$-score value per 

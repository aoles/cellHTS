...
\begin{eqnarray}
y_k &=&  \min_{i}\lbrace{-x'_{ki}}\rbrace \label{eq:replicatesSummary}\\
z_k &=& \frac{y_k-\hat{\mu}}{\hat{\sigma}}. \label{eq:defz}
\end{eqnarray}
Here, the summary is taken over all replicates $i$ for gene $k$. The 
minus sign on the right hand side of Equation~\eqref{eq:replicatesSummary}
...
For an activator assay, the minus sign is omitted. Thus, in both type of assays, large positive values of $y_k$ will correspond 
to a strong effect. By using the minimun as the summary function in Equation
\eqref{eq:replicatesSummary}, the analysis is particularly
conservative: all replicate values have to be high in order for $y_k$
to be high. Depending on the intended
stringency of the analysis, other plausible choices of summary
function are the mean and the maximum. 
In Equation~\eqref{eq:defz}, $\hat{\mu}$ and $\hat{\sigma}$ are the 
...
namely, the median for $\hat{\mu}$ and the median absolute deviation 
(mad) for $\hat{\sigma}$.
%
<<calcZscore>>=

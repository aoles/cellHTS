...
each gene. One option is to take the minimum of the $z$-scores from the two
replicates:
\begin{eqnarray}
z_{ki} &=& \pm \frac{x'_{ki}-\hat{\mu}}{\hat{\sigma}} \label{eq:defz}
z_{k}  &=& \min_{i} z_{ki} \label{eq:scoresSummary}.
\end{eqnarray}
...
replicates for probe $k$.  By using the minimun as the summary
function, the analysis is
particularly conservative: all replicate values have to be high in
order for $z_{k}$ to be high. Depending on the intended stringency of
the analysis, other plausible choices of summary function are the mean
and the maximum.
%
<<calcZscore>>=
...
\Rfunction{normalizePlateMedian}, and then do the summarization
between replicates in the function \Rfunction{summarizeZscore}.
%
<<normalizePlateMedianWithZscore>>=
xalt = normalizePlateMedian(x, zscore="-") 
xalt = summarizeZscore(xalt, summary="max")
@ 
%
...
%------------------------------------------------------------
\section{Summarization of replicates}
...
% This is how 'bdgpbiomart' it is created from 'x$geneAnno' 
% (before the assignment in the chunk <<bdgpbiomart>> above) from the 
% table 'bmAll' (which is produced by the call to Sweave("biomart.Rnw")
% below):
%
...
writeReport(x, force=TRUE, 
  plotPlateArgs   = list(xrange=c(-2, 6.5)),
...
sel = !is.na(x$score) & (!is.na(x$geneAnno$go_id))
goids = strsplit(x$geneAnno$go_id[sel], ", ")
genes = rep(x$geneAnno$GeneID[sel], listLen(goids))
categs = cache("categs", cateGOry(genes, unlist(goids, use.names=FALSE)))
@ 
% 

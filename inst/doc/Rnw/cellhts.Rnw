...
delimited text file, with mandatory columns \emph{Batch}, \emph{Position},
\emph{Well}, \emph{Content}. The \emph{Position} column runs from 1 to the
number of wells in the plate (in the example,
\Sexpr{max(x$plateConf$Position)}), and \emph{Well} is the name of the
...
each probe in each replicate $j$:
\begin{eqnarray}
...
the minimum of the $z$-scores from the two replicates:
\begin{eqnarray}
z_{k} &=&  \min_{j}z_{kj}. \label{eq:scoresSummary}
\end{eqnarray}
The summary is taken over all replicates for probe $k$.  
...
%
Since we want to produce the HTML quality reports (see
Section~\ref{sec:report}) using the normalized and
scored replicate values ($z_{kj}$), we will call again the
\Rfunction{normalizePlateMedian}, and choose the option to score 
the replicates after scaling by the plate median\footnote{The
\Rpackage{cellHTS} package also provides a function called 
\Rfunction{calcZscore} that
first summarizes the normalized values across replicates, and then calculates
a $z$-score value for each probe. However, in the present example, we prefer 
to have the normalized and scored replicates in \Robject{x\$xnorm}, and use them for 
constructing the quality report.}:
%
<<normalizePlateMedianandScore>>=
x = normalizePlateMedian(x, zscore="-") 
@ 
%
Note that this will replace the content in \Robject{x\$xnorm} by the scored
replicate values $z_{kj}$.
Then, using the function below, we summarize the scored replicates, 
obtaining a single $z$-score value per 
probe that will be stored in the slot \Robject{x\$score}:
%
<<summarizeZscore>>=
x = summarizeZscore(x, summary="min")
@ 
% 
The content of \Robject{x\$score} should always correspond to the final list of
scored probes. Boxplots of the $z$-scores for the different types of probes are shown
...
package or do not want to use it. In the remainder of this section, we
...
writeReport(x, force=TRUE, 
  plotPlateArgs = list(xrange=c(-6, 5)),
...
plate. The latter image plot can also be produced separately 
using the function \Rfunction{imageScreen} given in the \Rpackage{cellHTS}
package. This might be useful if we want to select the best display for our
data, namely, the aspect ratio for the plot and/or the range of $z$-score 
values to be mapped into the color scale. These can be passed to the function's arguments \Robject{ar} and \Robject{zrange}, respectively. For example,

<<imageScreen>>=
imageScreen(x, ar=1, zrange=c(-3,4))
@ 

It should be noted that the per-plate and per-experiment 
quality reports are constructed based on the content of
\Robject{x\$xnorm}, if it is present in the \Robject{x} object. 
Otherwise, it uses the content 
given in the slot \Robject{x\$xraw}. 
Although we have called the former slot, \Robject{xnorm}, it can
contain already scored replicate data (as in the present case; see 
Section~\ref{sec:scoring}), or, in the case of dual-channel experiments, 
the ratio between the intensities in two different channels, etc. 
